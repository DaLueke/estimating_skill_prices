...
with open(path.joinpath("OUT/rslt_dict_pp.json"), "w") as json_file:
    json.dump(rslt_dict, json_file)

# (2) difference between estimation and true price changes.
with open(path.joinpath("OUT/rslt_difference_dict_pp.json"), "w") as json_file:
    json.dump(rslt_difference_dict, json_file)

# (3) Count of corner solutions in task choice for one of the MC iterations.
with open(path.joinpath("OUT/count_corner_solutions_pp.json"), "w") as json_file:
